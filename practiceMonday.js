...
var threeSum2 = function(nums) {
  let LEN=nums.length
  let solutionArr = [];
  // iterate thru the array in 2 levels
  for (let i = 0; i < nums.length; i++) {
    console.log(i,' of ',LEN)
    for (let j = 0; j < nums.length; j++) {
      // TODO i should not be the same as j
      if (i !== j) {
        let copyOfNums = [...nums];
        let sumOfTheTwo = nums[i] + nums[j];
        let potentialSolutionArr = [nums[i], nums[j]];
        let indexFirst = copyOfNums.indexOf(nums[i]);
        copyOfNums.splice(indexFirst, 1);
        let indexSecond = copyOfNums.indexOf(nums[j]);
        copyOfNums.splice(indexSecond, 1);
        // assess if the 3rd item making the sum 0 is part of the "remaining array"
        if (copyOfNums.includes(0 - sumOfTheTwo)) {
          // if yes add that oneOfTheSolutionArr (values in that i and j) to the solutionArr
          //TODO The solution set must not contain duplicate triplets.
          //Check if the triplet is already in the solutionArr - if yes, do NOT push
          potentialSolutionArr.push(0 - sumOfTheTwo);
          let sortedPotentialSolution = potentialSolutionArr.sort((a, b) =>
            a > b ? 1 : -1
          );
          if (doesTripletExistInArr(sortedPotentialSolution, solutionArr)) {
            solutionArr.push(potentialSolutionArr);
          }
        }
      }
    }
...
var threeSum = function(nums) {
  ...
  // sort input and generate a hashtable for faster lookup
  ...
  for (let i = 0; i < LEN; i++) {
    for (let j = i; j < LEN; j++) {
      // TODO i should not be the same as j
      if (i !== j) {
        let potentialSolutionArr
        let target = 0-nums[i] - nums[j];
        hashMap[nums[i]]-=1 //make sure what if there are multiple values in the nums array" are handled in the correct way for decision
        hashMap[nums[j]]-=1
        // assess if the 3rd item making the sum 0 is part of the "remaining array"
        if (hashMap[target]>0){
          //Make sure the potentialSolutionArr is sorted "during the array generation"
          if (target<nums[i]){
            potentialSolutionArr = [target,nums[i],nums[j]];
          } else if (target>nums[j]){
            potentialSolutionArr = [nums[i], nums[j],target];
          } else {
            potentialSolutionArr = [nums[i], target,nums[j]];
          }
          let sortedPotentialSolutionString=potentialSolutionArr.join('') //create a "fingerprint" - and check against that - faster!
          if (!solutionArrHash[sortedPotentialSolutionString]){ //use to check "fingerprint db"
            solutionArrHash[sortedPotentialSolutionString]=1 //add to "fingerprint db"
            solutionArr.push(potentialSolutionArr) //push it to the solutionArr as well. this would remove the need to convert the hash to array when returning
          }
        }
        hashMap[nums[i]]+=1 //put it back to where it was
        hashMap[nums[j]]+=1
      }
    }
  }
...
class NodewidthOfBinaryTreeClass{
  constructor(value){
    this.val=value,
    this.left=null,
    this.right=null
...
var widthOfBinaryTree = function(root) {
  ...
  if (nextQueue[0]!==null && nextQueue[nextQueue.length-1]!==null && maxWidth<nextQueue.length){
    maxWidth=nextQueue.length
  }
