    ...
    for (let j = i; j < LEN; j++) {
      ...
      if (i !== j) {
        ...
        // assess if the 3rd item making the sum 0 is part of the "remaining array"
          ...
          let sortedPotentialSolutionString=sortedPotentialSolution.join('')

          if (!solutionArrHash[sortedPotentialSolutionString]){
            solutionArrHash[sortedPotentialSolutionString]=1
            solutionArr.push(sortedPotentialSolution)
          }
        }
        hashMap[nums[i]]+=1
        hashMap[nums[j]]+=1
